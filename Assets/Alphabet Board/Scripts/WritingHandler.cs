...
{
    public static WritingHandler THIS;
    public List<GameObject> activWinPartecl;
    public List<GameObject> activWin1Partecl;
    public List<GameObject> activWin2Partecl;
    public List<GameObject> activWin3Partecl;
    public static float hold = 4f;
    public int[] startPoints;
    private Buton button;
    public GameObject[] letters;//the letters list from A-Z
                                // public GameObject[] falseLetters;//the letters list from A-Z
    public static int currentFalseLetterIndex;//the index of the current letter
    public GameObject[] Hints;//the Hints list 
    public static int currentLetterIndex;//the index of the current letter
    public static int currentLetterHintIndex;//the index of the current letter
    public static int currentHintIndex;//the index of the current Hint
    ...
    public GameObject winParticle1;
    public GameObject winParticle2;
    public GameObject winParticle3;
    IEnumerator Start()
    {
        THIS = this;
        Cursor.visible = showCursor;//show curosr or hide
        currentTracingPoints = new ArrayList();//initiate the current tracing points
        Debug.Log(startPoints[currentLetterIndex]);
        currentHintIndex = startPoints[currentLetterIndex];
        currentLetterHintIndex = currentLetterIndex;
        Debug.Log(currentLetterHintIndex);
        LoadLetter();
        LoadHint();
        activWinPartecl = new List<GameObject>();
        activWin1Partecl = new List<GameObject>();
        activWin2Partecl = new List<GameObject>();
        activWin3Partecl = new List<GameObject>();
        yield return 0;
    }
    //Executes Every Single Frame
    void Update()
    {
        if (letterDone && hintDone)
        ...
        {//Touch Moved
            if (obTag == "TracingPoint")
            {
                ...
                int currentindex = tracingPoint.index;//get the tracing point index
                                                      //       Instantiate(hint,tracingPoint.transform.position , Quaternion.identity);
            ...
            InstaitaeParticl(currentPosition, currentLineRender.transform);
    ...
    private void EndTouchLetterHandle()
    {
        ...
        {//if the current letter done or wirrten successfully
            Debug.Log("letterDone");

            if (cheeringSound != null)
                AudioSource.PlayClipAtPoint(cheeringSound, Vector3.zero, 0.8f);//play the cheering sound effect
            pin.GetComponent<SpriteRenderer>().enabled = false;//hide the hand
            for (int i = 0; i <= 2; i++)
            {
                Invoke("InstantiateWinParticle", 0f);
            }
          //  Invoke("LoadNextLetter", hold);
           
            Invoke("SetButtonsAgain", hold + .1f);
            StartCoroutine(DeleteWinParticl());

        }
    }
    //Check letter done or not
    public void CheckLetterDone()
    {
    ...
    private void BackToMenu()
    {
        ...
        currentHintIndex = 0;
        currentLetterHintIndex = 0;
        currentFalseLetterIndex = 0;
    }
    ...
    private void RefreshLines()
    {
        ...
        GameObject[] gameobjs = GameObject.FindGameObjectsWithTag("LineRenderer");
        if (gameobjs == null)
        {
            return;
        }
        ...
        {
            Destroy(gob);
            currentHintIndex = startPoints[currentLetterHintIndex];
            LoadHint();
    ...
    public void PlayStartSound()
    {
        if (startSound != null)
        {
            AudioSource.PlayClipAtPoint(startSound, Vector3.zero, 180.0f);
        }
    }
    ...
    public void LoadNextLetter()
    {
        ...
        else if (currentLetterIndex >= 0 && currentLetterIndex < letters.Length - 1)
        {
            ...
            if (!letterDone)
            {
                currentFalseLetterIndex++;
                Debug.Log(currentFalseLetterIndex);
            }
            LoadLetter();
            LoadNixtHintIndex();
        }
    ...
    public void LoadPreviousLetter()
    {
        if (currentLetterIndex > 0 && currentLetterIndex < letters.Length)
        {
            currentLetterIndex--;
            LoadLetter();
            if (currentLetterHintIndex >= 0 && currentLetterHintIndex < startPoints.Length)
            {
                currentLetterHintIndex--;
                currentFalseLetterIndex--;
                Debug.Log(currentFalseLetterIndex);
                currentHintIndex = startPoints[currentLetterHintIndex];
                Hints[currentHintIndex].SetActive(true);
                Debug.Log(currentHintIndex);
            }
        }
    }
    ...
    private void InstaitaeCirclePoint(Vector3 position, Transform parent)
    {
        ...
        currentcicrle.GetComponent<Renderer>().material = currentLineRender.GetComponent<LineRenderer>().material;
        currentcicrle.transform.position = position;
    }
    private void InstaitaeParticl(Vector3 position, Transform parent)
    {
        GameObject partec = (GameObject)Instantiate(particle);//instaiate object
        partec.transform.parent = parent;
        partec.transform.position = position;
    }
    void InstantiateWinParticle()
    {
        activWinPartecl.Add(Instantiate(winParticle, new Vector3(-5.62f, -4.15f, -9), Quaternion.identity) as GameObject);
        activWin1Partecl.Add(Instantiate(winParticle1, new Vector3(-1.94f, -4.31f, -9), Quaternion.identity) as GameObject);
        activWin2Partecl.Add(Instantiate(winParticle2, new Vector3(3.17f, -4.44f, -9), Quaternion.identity) as GameObject);
        activWin3Partecl.Add(Instantiate(winParticle3, new Vector3(6.58f, -4.39f, -9), Quaternion.identity) as GameObject);
    }
    IEnumerator DeleteWinParticl()
    {
        yield return new WaitForSeconds(2.5f);
        foreach (GameObject item in activWinPartecl)
        {
            Debug.Log("Deleted");
            DestroyImmediate(item);
        }
        activWinPartecl.Clear();
        foreach (GameObject item in activWin1Partecl)
        {
            Debug.Log("Deleted");
            DestroyImmediate(item);
        }
        activWin1Partecl.Clear();
        foreach (GameObject item in activWin2Partecl)
        {
            Debug.Log("Deleted");
            DestroyImmediate(item);
        }
        activWin2Partecl.Clear();
        foreach (GameObject item in activWin3Partecl)
        {
            Debug.Log("Deleted");
            DestroyImmediate(item);
        }
        activWin3Partecl.Clear();
    }
    public void LoadNextHint()
    {
        ...
        else if (currentHintIndex >= 0 && currentHintIndex < Hints.Length - 1)
        {
            currentHintIndex++;
            LoadHint();
        }
    ...
    private void LoadHint()
    {
        ...
        if (Hints[currentHintIndex] == null)
        {
            return;
        }
        hintDone = false;
    ...
    public void RefreshHintProcess()
    {
        RefreshHints();
        TracingPart[] tracingParts = letters[currentLetterIndex].GetComponents<TracingPart>();
        foreach (TracingPart part in tracingParts)
    ...
    {
        GameObject[] gameobjs = GameObject.FindGameObjectsWithTag("Hint");
        if (gameobjs == null)
        {
            return;
        }
    ...
    public void LoadNixtHintIndex()
    {
        if (currentLetterHintIndex == startPoints.Length - 1)
        {
            currentHintIndex = 0;
            RefreshHintProcess();
        }
        else if (currentLetterHintIndex >= 0 && currentLetterHintIndex < startPoints.Length - 1)
        {
            currentLetterHintIndex++;
            currentHintIndex = startPoints[currentLetterHintIndex];
